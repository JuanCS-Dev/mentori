...
describe("Chaos Orchestrator", () => {
  ...
  describe("Experiment: PARTITION", () => {
    it("should throw error when partition is active", async () => {
      ...
      await expect(chaosPartition(operation, "test-partition")).rejects.toThrow(
    ...
    it("should return fallback if provided during partition", async () => {
      ...
      const operation = async () => "success";
      const result = await chaosPartition(
        operation,
        "test-partition-fallback",
        "fallback-value",
      );
  ...
  describe("Experiment: CRASH", () => {
    it("should throw error when crash is active", () => {
      ...
      expect(() => chaosCrash("test-component")).toThrow(
  ...
  describe("Resilience: Circuit Breaker", () => {
    it("should open after threshold failures", async () => {
      ...
      // 1st Failure
      try {
        await withCircuitBreaker(circuitName, failingOp, "fallback");
      } catch {
        /* ignore */
      }
      // 2nd Failure
