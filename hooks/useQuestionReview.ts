...
export function useQuestionReview() {
   ...
   */
  const getDueQuestionIds = useCallback(
    (limit?: number): string[] => {
   ...
   */
  const isQuestionDue = useCallback(
   ...
   */
  const getQuestionCard = useCallback(
    (questionId: string): QuestionReviewCard | null => {
  ...
  const stats = useMemo(() => {
    ...
    const byDiscipline: Record<
      string,
      { total: number; due: number; retention: number }
    > = {};
   ...
   */
  const prioritizeQuestions = useCallback(
    (questions: RealQuestion[]): RealQuestion[] => {
   ...
   */
  const applyInterleaving = useCallback(
    (
      questions: RealQuestion[],
      targetSwitchRate: number = 0.7,
    ): RealQuestion[] => {
      ...
      while (result.length < questions.length) {
        ...
        const currentSwitchRate =
          result.length > 1 ? switches / (result.length - 1) : 0;
          ...
          if (availableDisciplines.length > 0) {
            ...
            nextDiscipline =
              availableDisciplines[
                Math.floor(Math.random() * availableDisciplines.length)
              ] || lastDiscipline;
          }
        }
        ...
        if (
          !nextDiscipline ||
          (byDiscipline.get(nextDiscipline)?.length ?? 0) === 0
        ) {
