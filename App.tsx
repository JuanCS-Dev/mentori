...
// =============================================================================
const EditalAnalyzer = React.lazy(() =>
  import("./features/EditalAnalyzer").then((module) => ({
    default: module.EditalAnalyzer,
  })),
);
const BankProfiler = React.lazy(() =>
  import("./features/BankProfiler").then((module) => ({
    default: module.BankProfiler,
  })),
);
const StudyPlanner = React.lazy(() =>
  import("./features/StudyPlanner").then((module) => ({
    default: module.StudyPlanner,
  })),
);
const QuestionBank = React.lazy(() =>
  import("./features/QuestionBank").then((module) => ({
    default: module.QuestionBank,
  })),
);
const MaterialGenerator = React.lazy(() =>
  import("./features/MaterialGenerator").then((module) => ({
    default: module.MaterialGenerator,
  })),
);
const DiscursiveMentor = React.lazy(() =>
  import("./features/DiscursiveMentor").then((module) => ({
    default: module.DiscursiveMentor,
  })),
);
const ProgressDashboard = React.lazy(() =>
  import("./features/ProgressDashboard").then((module) => ({
    default: module.ProgressDashboard,
  })),
);
const StudyCycle = React.lazy(() =>
  import("./features/StudyCycle").then((module) => ({
    default: module.StudyCycle,
  })),
);
const WeeklyReport = React.lazy(() =>
  import("./features/WeeklyReport").then((module) => ({
    default: module.WeeklyReport,
  })),
);
const SalesLanding = React.lazy(() =>
  import("./features/SalesLanding").then((module) => ({
    default: module.SalesLanding,
  })),
);
const DashboardHub = React.lazy(() =>
  import("./features/DashboardHub").then((module) => ({
    default: module.DashboardHub,
  })),
);
...
const App: React.FC = () => {
  const [currentView, setCurrentView] = useState<AppView>(
    AppView.SALES_LANDING,
  );

  // Persisted State (survives page refresh)
  const [editalData, setEditalData] = usePersistence<EditalJSON | null>(
    "editalData",
    null,
  );
  const [profileData, setProfileData] = usePersistence<BankProfileJSON | null>(
    "profileData",
    null,
  );
  const [studyPlan, setStudyPlan] = usePersistence<NeuroStudyPlanJSON | null>(
    "studyPlan",
    null,
  );
  const [userMood] = usePersistence<"focused" | "tired" | "anxious">(
    "userMood",
    "focused",
  );
  ...
  useEffect(() => {
    if (explanationState.isGenerating) {
      console.warn(
        `🧠 Generating explanations... ${explanationState.questionsProcessed} done, ${explanationState.questionsRemaining} remaining`,
      );
    }
  }, [
    explanationState.isGenerating,
    explanationState.questionsProcessed,
    explanationState.questionsRemaining,
  ]);
          ...
          <Layout currentView={currentView} onNavigate={setCurrentView}>
            <Suspense fallback={<LoadingFallback />}>
              ...
              {currentView === AppView.EDITAL && (
                <EditalAnalyzer
                  onDataUpdate={setEditalData}
                  savedData={editalData}
                />
              )}

              {currentView === AppView.PROFILE && (
                <BankProfiler
                  onDataUpdate={setProfileData}
              ...
              {currentView === AppView.PROGRESS && (
                <ProgressDashboard userMood={userMood} studyPlan={studyPlan} />
              )}

              {currentView === AppView.STUDY_CYCLE && (
                <StudyCycle editalData={editalData} />
              )}

              {currentView === AppView.QUESTIONS && <QuestionBank />}

              {currentView === AppView.MATERIAL && (
                <MaterialGenerator
                  bankProfile={profileData ? JSON.stringify(profileData) : ""}
                />
              )}

              {currentView === AppView.DISCURSIVE && editalData && (
                <DiscursiveMentor metadata={editalData.metadata} />
              )}
              ...
              {currentView === AppView.DISCURSIVE && !editalData && (
                <div className="text-center py-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="bg-white p-10 rounded-xl inline-block shadow-sm border border-kitchen-border">
                    ...
                    <h2 className="text-xl font-bold font-mono text-gray-800 mb-2">
                      Acesso Negado
                    </h2>
                    <p className="text-gray-500 mb-8 max-w-md font-mono text-sm">
                      Erro: Dados do edital não encontrados. Por favor, execute
                      Edital_Analyzer() primeiro.
                    </p>
                    <button
              ...
              {currentView === AppView.WEEKLY_REPORT && (
                <WeeklyReport
                  ...
                  onNavigateToDiscipline={(_discipline) => {
