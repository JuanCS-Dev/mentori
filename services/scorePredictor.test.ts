...
describe("ScorePredictor", () => {
  describe("predictScore", () => {
    ...
    it("should generate focus recommendations", () => {
      ...
      const direitoPenal = prediction.focusRecommendations.find(
        (r) => r.disciplina === "Direito Penal",
      );
  ...
  describe("calculateDisciplineBreakdown", () => {
    it("should categorize disciplines by status", () => {
      ...
      const breakdown = ScorePredictor.calculateDisciplineBreakdown(
        performances,
        100,
      );

      expect(breakdown.find((d) => d.disciplina === "Strong")?.status).toBe(
        "strong",
      );
      expect(breakdown.find((d) => d.disciplina === "Average")?.status).toBe(
        "average",
      );
      expect(breakdown.find((d) => d.disciplina === "Weak")?.status).toBe(
        "weak",
      );
      expect(breakdown.find((d) => d.disciplina === "Critical")?.status).toBe(
        "critical",
      );
    });
  });
