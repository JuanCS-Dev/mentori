...
export const DifficultySelector: React.FC<DifficultySelectorProps> = ({
  ...
  return (
    <div
      className={`flex items-center gap-4 bg-white/50 p-3 rounded-xl border border-slate-200 ${className}`}
    >
      <div
        className={`p-2 rounded-lg ${levelColor
          .replace("text-", "bg-")
          .replace("600", "100")}`}
      >
      ...
      <div className="flex-1">
        ...
        <div className="flex items-baseline gap-2 mt-1">
          <h3 className={`text-lg font-bold ${levelColor}`}>{levelTitle}</h3>
          <span className="text-xs text-slate-400">(Top {winProb}% est.)</span>
        </div>

        {/* Visual Bar */}
        <div className="mt-2 h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
          <div
            className={`h-full ${levelColor.replace(
              "text-",
              "bg-",
