...
export const StudyPlanner: React.FC<Props> = ({
  ...
  const handleGenerate = async () => {
    setLoading(true);
    try {
      const plan = await NebiusService.generateNeuroStudyPlan({
        ...anamnese,
    ...
    <div className="bg-white border border-kitchen-border p-8 rounded-xl max-w-4xl mx-auto relative overflow-hidden animate-in zoom-in-95 duration-500 shadow-sm">
      ...
      <div className="mb-8 mt-4 text-center">
        <h2 className="text-2xl font-mono font-bold text-slate-800">
          Calibragem Neuro-Cognitiva
        </h2>
        <p className="text-slate-500 font-mono text-sm mt-2">
          Para criar o plano perfeito, precisamos entender sua biologia.
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            ...
            <div className="space-y-4">
              <label className="font-bold font-mono text-sm text-slate-700 flex items-center gap-2">
                <Sun size={18} className="text-orange-500" /> Seu Cronotipo
                (Pico de Energia)
              </label>
              <div className="flex flex-col gap-3">
                {["Matutino", "Vespertino", "Noturno"].map((type) => (
                  <button
                    key={type}
                    onClick={() =>
                      setAnamnese({ ...anamnese, cronotipo: type })
                    }
            ...
            <div className="space-y-4">
              <label className="font-bold font-mono text-sm text-slate-700 flex items-center gap-2">
                <Battery size={18} className="text-blue-500" /> Ciclo de Foco
                (Ultradiano)
              </label>
              <div className="flex gap-3">
                {[30, 50, 90].map((mins) => (
                  <button
                    key={mins}
                    onClick={() =>
                      setAnamnese({ ...anamnese, foco_maximo: mins })
                    }
              ...
              <p className="text-xs text-slate-400 font-mono mt-2 text-center">
                Tempo que consegue estudar sem perder a atenção.
              </p>
            </div>
          </div>
        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              <label className="font-bold font-mono text-sm text-slate-700 flex items-center gap-2">
                <Activity size={16} className="text-rose-500" /> NÍVEL DE
                ESTRESSE
              </label>
              <div className="grid grid-cols-3 gap-3">
                {["Baixo", "Médio", "Alto"].map((level) => (
                  <button
                    key={level}
                    onClick={() =>
                      setAnamnese({ ...anamnese, nivel_estresse: level })
                    }
                    className={`p-4 rounded-lg border text-sm font-bold font-mono transition-all ${
                      anamnese.nivel_estresse === level
            ...
            <div className="space-y-3">
              <label className="font-bold font-mono text-sm text-slate-700 flex items-center gap-2">
                <Clock size={16} className="text-blue-500" /> HORAS LÍQUIDAS
              </label>
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
                <input
                  ...
                  onChange={(e) =>
                    setAnamnese({
                      ...anamnese,
                      horas_diarias: Number(e.target.value),
                    })
                  }
                  className="w-full accent-blue-600 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
                />
                <div className="text-center font-mono font-bold text-3xl text-blue-600 mt-4">
                  {anamnese.horas_diarias}h
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-8 animate-in slide-in-from-bottom-8 duration-700">
          ...
          <div className="bg-white border border-kitchen-border rounded-xl p-8 shadow-sm flex flex-col md:flex-row gap-8 items-center">
            ...
            <div className="flex-1 text-center md:text-left">
              <h2 className="text-2xl font-mono font-bold text-slate-800 mb-2">
                Diagnóstico Bio-Adaptativo
              </h2>
              <p className="text-slate-500 font-mono text-sm">
                {savedPlan.diagnostico.perfil_cognitivo}
              </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
              <h3 className="font-bold text-slate-800 flex items-center gap-2 mb-6">
                <Sun className="text-orange-500" /> Protocolo Matinal
              </h3>
            ...
            <div className="lg:col-span-2 space-y-4">
              <h3 className="font-bold text-slate-800 flex items-center gap-2 mb-2 px-2">
                <Activity className="text-indigo-500" /> Blocos de Alta
                Performance
              </h3>
              {savedPlan.blocos_estudo.map((block, i: number) => (
                <div
                  key={i}
                  className={`p-5 rounded-xl border border-kitchen-border bg-white shadow-sm flex items-center gap-6 transition-all hover:shadow-md ${
                ...
                >
                  <div className="text-center min-w-[80px]">
                    <div className="font-mono font-bold text-slate-800">
                      {block.horario.split(" - ")[0]}
                    </div>
                    <div className="text-xs font-mono text-slate-400">
                      {block.horario.split(" - ")[1]}
                    </div>
                  </div>
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-slate-500 mt-1 font-mono bg-gray-50 inline-block px-2 py-1 rounded border border-gray-100">
                      {block.metodo}
                    </p>
                    {block.motivo && (
                      <p className="text-xs text-emerald-600 mt-2 italic flex items-center gap-1">
                        <Brain size={12} /> {block.motivo}
                      </p>
                    )}
                  </div>
                  {block.energia_exigida === "Alta" ? (
                    <BatteryCharging className="text-indigo-400" size={20} />
                  ) : (
          ...
          <div className="bg-white border border-kitchen-border rounded-xl p-8 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              <h3 className="font-bold text-xl mb-2 flex items-center gap-2 text-slate-800">
                <Award className="text-purple-500" /> Gatilhos de Dopamina
              </h3>
              <p className="text-slate-500 text-sm">
                O cérebro precisa de recompensa para criar o hábito.
              </p>
            </div>
            <div className="text-right flex flex-wrap gap-2 justify-end">
              {savedPlan.dopamina_triggers.map((trigger: string, i: number) => (
                <div
                  key={i}
                  className="font-mono font-bold text-xs bg-purple-50 text-purple-700 px-4 py-2 rounded-lg border border-purple-100"
                >
                  {trigger}
                </div>
              ))}
