...
export const SimuladoControls: React.FC<SimuladoControlsProps> = ({
  dbMeta,
  loading,
  userAccuracy = 0,
  onStart,
  ...
  const [config, setConfig] = React.useState<
    Omit<SimuladoConfig, "level" | "useAI">
  >({
    questionCount: 10,
    timeLimit: 0,
    discipline: "",
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        {(
          Object.entries(SIMULADO_LEVELS) as [
            SimuladoLevel,
            typeof SIMULADO_LEVELS.treino,
          ][]
        ).map(([key, lvl]) => {
          const Icon = lvl.icon;
            ...
            >
              ...
              >
                <Icon
                  size={18}
                  className={
                    isSelected ? `text-${lvl.color}-600` : "text-gray-500"
                  }
                />
              </div>
              <h4
        ...
        <div className="flex items-center justify-between pt-4 mt-4 border-t border-gray-100">
          ...
          {userAccuracy > 0 && (
            <span className="text-xs text-gray-500 font-mono">
              Seu desempenho:{" "}
              <span className="font-bold text-gray-700">{userAccuracy}%</span>
            </span>
          )}
        </div>
      </div>
