...
export const DiscursiveMentor: React.FC<Props> = ({ metadata }) => {
  ...
  const [evaluation, setEvaluation] = useState<DiscursiveEvaluation | null>(
    null,
  );
  ...
  const startBattle = async () => {
    setLoading(true);
    try {
      const t = await NebiusService.generateDiscursiveTheme(
  ...
  const submitForCorrection = async () => {
    ...
    setLoading(true);
    try {
      const result = await NebiusService.evaluateDiscursive(
        theme!,
        studentText,
        metadata?.banca || "FGV",
      );
      ...
      {!theme && !loading && (
        <div className="bg-white border border-kitchen-border rounded-xl p-16 text-center shadow-sm">
          ...
          <h2 className="text-3xl font-extrabold text-slate-800 mb-4">
            Sala de Batalha Discursiva
          </h2>
          <p className="text-slate-500 max-w-xl mx-auto text-lg mb-10">
            Treine com temas inéditos gerados pelo DNA da sua banca. Receba
            correção instantânea com barema oficial de 2025.
          </p>
          <button
            onClick={startBattle}
      ...
      {loading && (
        <div className="py-24 text-center">
          ...
          <p className="text-xl font-bold text-slate-700">
            O Mentor está preparando sua prova...
          </p>
        </div>
      )}
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white border border-kitchen-border p-8 rounded-xl shadow-sm relative overflow-hidden">
              <div className="flex justify-between items-center mb-8">
                <span className="px-3 py-1 bg-green-50 text-green-700 rounded-full text-[10px] font-bold uppercase tracking-widest border border-green-100">
                  Simulado Ativo
                </span>
                <div
              ...
              <div className="prose prose-sm text-slate-600 mb-8 max-h-[400px] overflow-y-auto pr-4 scrollbar-hide">
                <p className="font-medium text-slate-800 mb-4 italic border-l-4 border-slate-200 pl-4">
                  "{theme.enunciado}"
                </p>
            ...
            <div className="bg-white border border-kitchen-border rounded-xl p-8 flex flex-col h-[700px] shadow-sm">
              <div className="flex justify-between items-center mb-6">
                ...
                <span className="text-xs font-mono text-gray-400 bg-gray-50 px-2 py-1 rounded">
                  {studentText.length} caracteres • ~
                  {Math.ceil(studentText.length / 70)} linhas
                </span>
              </div>
              <textarea
              ...
              <div className="mt-6 flex justify-end gap-4">
                <button
                  ...
                  className="px-6 py-2 text-gray-500 font-bold hover:text-gray-700 hover:bg-gray-50 rounded-lg transition-colors text-sm"
                >
                  Desistir
                </button>
                <button
          ...
          <div className="bg-white border border-kitchen-border rounded-xl p-8 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div className="md:col-span-3 grid grid-cols-3 gap-6">
              ...
              <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                ...
                <div className="text-xl font-mono font-bold text-slate-800">
                  {evaluation.pontuacao.estrutura}/30
                </div>
                <div className="h-1.5 w-full bg-gray-200 rounded-full mt-3 overflow-hidden">
                  <div
                    className="h-full bg-purple-500"
                    style={{
                      width: `${(evaluation.pontuacao.estrutura / 30) * 100}%`,
                    }}
                  ></div>
                </div>
              </div>
              <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                ...
                <div className="text-xl font-mono font-bold text-slate-800">
                  {evaluation.pontuacao.gramatica}/20
                </div>
                <div className="h-1.5 w-full bg-gray-200 rounded-full mt-3 overflow-hidden">
                  <div
                    className="h-full bg-emerald-500"
                    style={{
                      width: `${(evaluation.pontuacao.gramatica / 20) * 100}%`,
                    }}
                  ></div>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-white border border-kitchen-border rounded-xl p-8 shadow-sm">
              <h3 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2">
                <Award size={20} className="text-indigo-600" /> Parecer do
                Especialista
              </h3>
              ...
              <div className="mt-8 space-y-4">
                ...
                {evaluation.erros_identificados.map((erro, i: number) => (
                  ...
                  >
                    <p className="text-xs font-bold text-red-700 line-through mb-1 opacity-70">
                      {erro.trecho}
                    </p>
            ...
            <div className="bg-white border border-kitchen-border rounded-xl p-8 shadow-sm">
              ...
              <div className="mt-8 pt-6 border-t border-gray-100 flex justify-between items-center">
                <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">
                  Modelo de Resposta Nota 100
                </span>
                <button
                  onClick={startBattle}
                  className="bg-slate-900 text-white px-6 py-2 rounded-lg font-bold text-xs flex items-center gap-2 hover:bg-slate-800 transition-colors shadow-sm"
                >
