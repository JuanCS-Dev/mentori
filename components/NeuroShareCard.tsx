...
export const NeuroShareCard: React.FC<NeuroShareCardProps> = ({
  plan,
  stats = { focusMinutes: 45, streakDays: 3, questionsSolved: 12 },
  userMood,
}) => {
  ...
  const handleShare = async () => {
    if (navigator.share) {
      ...
      alert(
        "Opção de compartilhamento nativo não suportada neste browser. (Implementar html2canvas aqui)",
      );
    }
  };
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      {/* THE CARD ITSELF */}
      <div
        className={`relative w-[320px] h-[500px] rounded-[2rem] overflow-hidden bg-gradient-to-br ${getMoodColor()} shadow-2xl flex flex-col p-6 text-white border-4 border-white/10 group hover:scale-[1.02] transition-transform duration-500`}
      >
        ...
        <div className="relative z-10 text-center space-y-2 mb-8">
          ...
          <div className="text-7xl font-black tracking-tighter drop-shadow-lg">
            {stats.focusMinutes}
            <span className="text-3xl font-medium text-white/50">min</span>
          </div>
        ...
        <div className="relative z-10 mt-6 pt-6 border-t border-white/10 text-center">
          <p className="text-sm font-medium italic text-white/80">
            "
            {plan?.diagnostico?.estrategia_adotada ||
              "Construindo neuro-plasticidade..."}
            "
          </p>
