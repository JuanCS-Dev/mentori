  ...
  const processLine = (line: string, index: number) => {
    ...
    if (line.startsWith("# "))
      return (
        <h1
          key={index}
          className="text-3xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-slate-700 mt-8 mb-4 border-b border-slate-200/60 pb-3"
        >
          {line.replace("# ", "")}
        </h1>
      );
    if (line.startsWith("## "))
      return (
        <h2
          key={index}
          className="text-2xl font-bold text-slate-800 mt-6 mb-3 flex items-center gap-2"
        >
          {line.replace("## ", "")}
        </h2>
      );
    if (line.startsWith("### "))
      return (
        <h3
          key={index}
          className="text-lg font-bold text-indigo-700 mt-5 mb-2 uppercase tracking-wide"
        >
          {line.replace("### ", "")}
        </h3>
      );
    ...
    if (
      ...
      return (
        <div
          key={index}
          className="flex items-start gap-3 mt-6 mb-2 p-3 rounded-lg bg-slate-50 border border-slate-100"
        >
    ...
    if (line.trim().startsWith("- ") || line.trim().startsWith("□ ")) {
      return (
        <li
          key={index}
          className="ml-2 text-slate-600 list-none flex items-start py-1 group hover:text-slate-900 transition-colors"
        >
