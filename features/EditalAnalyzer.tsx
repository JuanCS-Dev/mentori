...
export const EditalAnalyzer: React.FC<Props> = ({
  onDataUpdate,
  savedData,
}) => {
  const [inputText, setInputText] = useState("");
  const [loading, setLoading] = useState(false);
  const [activeTab, setActiveTab] = useState<"overview" | "vertical" | "print">(
    "overview",
  );
  ...
  const handleAnalyze = async () => {
    ...
    setLoading(true);
    try {
      const result = await NebiusService.analyzeEdital(inputText);
      onDataUpdate(result);
    } catch (error) {
      console.error(error);
    ...
    return (
      <div className="bg-white min-h-screen text-slate-900 font-serif p-16 max-w-4xl mx-auto print:p-0 print:max-w-none">
        <div className="flex justify-between items-start mb-12 border-b-4 border-slate-900 pb-6 print:border-black">
          <div>
            <h1 className="text-4xl font-bold uppercase tracking-wide mb-2">
              {savedData.metadata.orgao}
            </h1>
            <h2 className="text-xl text-slate-600 font-sans uppercase tracking-widest">
              {savedData.metadata.banca} • {savedData.metadata.cargos[0]}
            </h2>
          </div>
          <div className="text-right">
            <button
              onClick={() => setActiveTab("overview")}
              className="print:hidden bg-slate-100 hover:bg-slate-200 px-4 py-2 rounded mb-2 block w-full text-sm font-sans font-bold text-slate-600"
            >
              Voltar
            </button>
            <button
              onClick={() => window.print()}
              className="print:hidden bg-slate-900 text-white px-4 py-2 rounded block w-full text-sm font-sans font-bold"
            >
              Imprimir PDF
            </button>
          </div>
            ...
            <ul className="space-y-3">
              {savedData.cronograma.map((c, i) => (
                <li key={i} className="flex justify-between text-sm">
                  <span
                    className={
                      c.critical ? "font-bold text-red-700" : "text-slate-700"
                    }
                  >
                    {c.evento}
                  </span>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {savedData.alertas.map((a, i) => (
                <li
                  key={i}
                  className="text-sm text-slate-700 bg-red-50 p-2 border-l-2 border-red-500 italic"
                >
                  "{a}"
                </li>
              ))}
        ...
        <h3 className="text-2xl font-bold mb-6 flex items-center gap-3">
          <span className="bg-slate-900 text-white w-8 h-8 flex items-center justify-center rounded text-sm font-sans">
            01
          </span>
            ...
            <div key={i} className="break-inside-avoid">
              <div className="flex justify-between items-end mb-3 border-b-2 border-slate-200 pb-2">
                ...
                <span className="text-sm font-mono bg-slate-100 px-2 py-1 rounded">
                  Peso: {disc.peso} | {disc.questoes} Questões
                </span>
              </div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      {!savedData && (
        <div className="bg-white border border-kitchen-border rounded-xl p-8 relative overflow-hidden shadow-sm">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1">
                Cole o texto ou faça upload do documento.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div
              className={`lg:col-span-1 rounded-2xl border-2 border-dashed transition-all duration-300 flex flex-col items-center justify-center p-6 text-center cursor-pointer group relative overflow-hidden ${
                dragActive
      ...
      {savedData && (
        <div className="bg-white border border-kitchen-border rounded-xl overflow-hidden min-h-[800px] flex flex-col shadow-sm">
          {/* Header */}
          <div className="bg-slate-900 p-8 text-white flex justify-between items-start">
            <div>
              ...
              <p className="text-indigo-300 font-medium">
                {savedData.metadata.cargos.join(", ")}
              </p>
            </div>
            <div className="text-right">
              <p className="text-2xl font-bold text-emerald-400">
                {savedData.metadata.remuneracao}
              </p>
          ...
          {/* Navigation */}
          <div className="bg-gray-50 border-b border-gray-200 px-8 flex gap-2 pt-2">
            <button
              onClick={() => setActiveTab("overview")}
              className={`px-4 py-2 rounded-t-lg text-sm font-bold font-mono transition-colors ${
                activeTab === "overview"
                  ? "bg-white border-x border-t border-kitchen-border text-blue-600 relative top-[1px]"
            ...
            <button
              ...
              className={`ml-auto px-4 py-2 text-sm font-bold font-mono text-gray-500 hover:text-blue-600 flex items-center gap-2`}
            >
            ...
            {activeTab === "overview" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  ...
                  <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                    <h3 className="flex items-center gap-2 font-bold text-slate-800 mb-4">
                      <Scale className="text-purple-500" size={20} /> Regras do
                      Jogo
                    </h3>
                    <div className="space-y-3">
                      {savedData.fases.map((fase, idx) => (
                        <div
                          key={idx}
                          className="p-3 border-l-4 border-purple-500 bg-purple-50/50"
                        >
                    ...
                    <div className="p-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {disc.topicos.map((topico, tIdx) => (
                          <label
                            key={tIdx}
                            className="flex items-start gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer border border-transparent hover:border-slate-200 group"
                          >
                            <input
                              type="checkbox"
