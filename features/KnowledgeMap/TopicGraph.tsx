...
export const KnowledgeMap: React.FC<KnowledgeMapProps> = ({
  ...
  const nodes = useMemo<TopicNode[]>(() => {
    return Object.entries(progress.disciplineStats).map(
      ([discipline, stats]) => ({
  ...
  if (nodes.length === 0) {
    return (
      <div
        className={`p-8 text-center text-slate-400 bg-slate-50 rounded-2xl ${className}`}
      >
        <div className="text-4xl mb-4">🧠</div>
        <p className="font-medium">Seu Mapa de Conhecimento está vazio</p>
        <p className="text-sm mt-1">
          Comece a estudar para visualizar seu domínio!
        </p>
      </div>
    );
  }
    ...
    <div className={`relative ${className}`}>
      ...
      <div className="flex justify-center gap-4 mb-4 text-xs">
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded-full bg-emerald-500"></span> Dominado
        </span>
        <span className="flex items-center gap-1">
        ...
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded-full bg-orange-400"></span> Atenção
        </span>
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded-full bg-red-400"></span> Crítico
        </span>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="text-xs text-slate-500 mt-1">
                {getMasteryLabel(node.mastery)} • {node.questionsAnswered}{" "}
                questões
              </div>
            </div>
          );
