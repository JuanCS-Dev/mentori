...
describe("StudyScheduler", () => {
  ...
  describe("generateDailySchedule", () => {
    ...
    it("should generate schedule for a weekday", () => {
      ...
      const schedule = StudyScheduler.generateDailySchedule(
        mockDisciplines,
        mockConfig,
        monday,
      );
      ...
      expect(schedule.totalMinutes).toBeLessThanOrEqual(
        mockConfig.dailyAvailableHours * 60,
      );
    });
    ...
    it("should distribute time proportionally to weights", () => {
      ...
      const d1Minutes = schedule.blocks
        .filter((b) => b.disciplineId === "d1")
        .reduce((sum, b) => sum + b.durationMinutes, 0);
      const d2Minutes = schedule.blocks
        .filter((b) => b.disciplineId === "d2")
        .reduce((sum, b) => sum + b.durationMinutes, 0);
  ...
  describe("calculateGoalProgress", () => {
    ...
    it("should calculate progress correctly when on track", () => {
      ...
      const progress = StudyScheduler.calculateGoalProgress(
        mockConfig,
        disciplines,
        720,
      );
  ...
  describe("suggestNextBlock", () => {
    ...
    it("should prioritize low Elo disciplines", () => {
      ...
      const suggested = StudyScheduler.suggestNextBlock(
        disciplines,
        performanceData,
      );
