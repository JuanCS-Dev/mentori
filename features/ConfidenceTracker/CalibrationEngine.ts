...
export function calculateCalibrationScore(logs: ConfidenceLog[]): number {
  ...
  for (let i = 0; i < bins.length - 1; i++) {
    ...
    const binLogs = logs.filter(
      (l) => l.confidence >= min && l.confidence < max,
    );

    if (binLogs.length > 0) {
      const avgConfidence =
        binLogs.reduce((acc, l) => acc + l.confidence, 0) /
        binLogs.length /
        100;
      const accuracy =
        binLogs.filter((l) => l.wasCorrect).length / binLogs.length;
...
export function getCalibrationFeedback(score: number): string {
  if (score > 0.9)
    return "🎯 Mestre da Metacognição! Você sabe exatamente o que sabe.";
  if (score > 0.8) return "🧠 Ótima calibração. Sua autoavaliação é confiável.";
  if (score > 0.6)
    return "⚠️ Atenção: Você tende a confiar demais ou de menos.";
