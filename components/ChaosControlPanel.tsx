...
export function ChaosControlPanel() {
  ...
  const toggleExperiment = (
    name: "LATENCY" | "PARTITION" | "CRASH" | "CORRUPTION",
  ) => {
    ...
    if (currentExp) {
      ...
      newExperiments[name] = {
        probability: 0.5,
        config: name === "LATENCY" ? { delayMs: 2000 } : undefined,
      };
    }
      ...
      <div className="overflow-y-auto p-4 space-y-6">
        ...
        <div className="flex items-center justify-between bg-gray-800/50 p-3 rounded-lg border border-gray-700">
          <div className="flex flex-col">
            ...
            <span className="text-xs text-gray-400">
              {config.enabled ? "SYSTEM UNSTABLE" : "System Normal"}
            </span>
          </div>
          <button
            onClick={toggleChaos}
            className={`w-12 h-6 rounded-full transition-colors relative ${
              config.enabled ? "bg-red-600" : "bg-gray-600"
            }`}
          >
            <div
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-2">
            {(["LATENCY", "PARTITION", "CRASH", "CORRUPTION"] as const).map(
              (type) => (
                <button
                  key={type}
                  onClick={() => toggleExperiment(type)}
                  className={`p-2 rounded border text-xs font-medium flex items-center justify-between ${
                    config.experiments[type]
          ...
          {health.length === 0 ? (
            ...
            <div className="space-y-2">
              {health.map((signal, i) => (
                <div
                  key={i}
                  className="text-xs flex items-center justify-between p-2 bg-gray-800/50 rounded border-l-2 border-gray-700"
                  style={{
                    borderLeftColor:
                ...
                >
                  ...
                  >
                    {signal.service.replace("sovereign:", "")}
                  </span>
                  <div className="flex items-center gap-2">
                    {signal.latency && (
                      <span className="text-gray-400">{signal.latency}ms</span>
                    )}
                    <span
          ...
          {Object.keys(circuits).length === 0 ? (
            ...
            <div className="space-y-2">
              {Object.entries(circuits).map(([name, state]) => (
                <div
                  key={name}
                  className="p-2 bg-gray-800/50 rounded border border-gray-700 flex flex-col gap-1"
                >
